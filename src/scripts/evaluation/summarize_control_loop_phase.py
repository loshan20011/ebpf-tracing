...
def main() -> int:
    ...
    lines = [
        "| Case | Dominant Target | Expected Behavior | Actual Behavior | Replica Behavior | SLO Result | Actions | Oscillation | Peak Replicas | Overall |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in rows:
        lines.append(
            "| {case} | {target} | {expected_behavior} | {actual_pattern} | {replica} | {slo} | {actions} | {oscillation} | {peak} | {overall} |".format(
                case=row["case_name"],
