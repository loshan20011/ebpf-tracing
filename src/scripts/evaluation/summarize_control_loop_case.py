...
def summarize(case_dir: Path, phase_name: str) -> dict:
    ...
    total_time_above_slo_seconds = round(violation_window_count * interval_seconds, 3)
    ...
    client_timeseries = bucket_requests(request_rows, bucket_seconds=10)

    actual_pattern_behavior = (
        f"{len(scale_up_actions)} scale-up and {len(scale_down_actions)} scale-down actions; "
    ...
    summary_md = "\n".join(
        [
            ...
            f"- Scale actions: {summary['control_loop']['scale_action_count']}",
